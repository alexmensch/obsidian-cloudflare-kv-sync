import { Plugin, TFile, Notice, PluginSettingTab, App, Setting, parseYaml, requestUrl } from 'obsidian';
...
export default class CloudflareKVPlugin extends Plugin {
  ...
  async syncAllTaggedFiles() {
    ...
    if (syncedFiles.length === 0) {
      new Notice(`No files found with '${this.settings.syncKey}: true' and an ID`);
      return;
    }
  ...
  validateSettings(): boolean {
    if (!this.settings.accountId || !this.settings.namespaceId || !this.settings.apiToken) {
